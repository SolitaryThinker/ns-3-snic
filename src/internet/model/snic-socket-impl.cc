...
NS_LOG_COMPONENT_DEFINE("SnicSocketImpl");

NS_OBJECT_ENSURE_REGISTERED(SnicSocketImpl);
...
static const uint32_t MAX_IPV4_SNIC_DATAGRAM_SIZE = 65507; //!< Maximum UDP datagram size

// Add attributes generic to all UdpSockets to base class SnicSocket
TypeId
SnicSocketImpl::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::SnicSocketImpl")
            .SetParent<SnicSocket>()
            .SetGroupName("Internet")
            .AddConstructor<SnicSocketImpl>()
            .AddTraceSource("Drop",
                            "Drop UDP packet due to receive buffer overflow",
                            MakeTraceSourceAccessor(&SnicSocketImpl::m_dropTrace),
                            "ns3::Packet::TracedCallback")
            .AddAttribute("IcmpCallback",
                          "Callback invoked whenever an icmp error is received on this socket.",
                          CallbackValue(),
                          MakeCallbackAccessor(&SnicSocketImpl::m_icmpCallback),
                          MakeCallbackChecker())
            .AddAttribute("IcmpCallback6",
                          "Callback invoked whenever an icmpv6 error is received on this socket.",
                          CallbackValue(),
                          MakeCallbackAccessor(&SnicSocketImpl::m_icmpCallback6),
                          MakeCallbackChecker());
    return tid;
}

SnicSocketImpl::SnicSocketImpl()
    : m_endPoint(nullptr),
      m_endPoint6(nullptr),
      m_node(nullptr),
      m_snic(nullptr),
...
SnicSocketImpl::~SnicSocketImpl()
{
     ...
     * SnicSocketImpl::Destroy or SnicSocketImpl::Destroy6
    ...
    if (m_endPoint != nullptr)
    {
         ...
         * in turn a call to the method SnicSocketImpl::Destroy below
         * will will zero the m_endPoint field.
         */
        NS_ASSERT(m_endPoint != nullptr);
        m_snic->DeAllocate(m_endPoint);
        NS_ASSERT(m_endPoint == nullptr);
    }
...
void
SnicSocketImpl::SetNode(Ptr<Node> node)
{
...
void
SnicSocketImpl::SetSnic(Ptr<SnicL4Protocol> snic)
{
    NS_LOG_FUNCTION(this << snic);
    m_snic = snic;
}

enum Socket::SocketErrno
SnicSocketImpl::GetErrno() const
{
    NS_LOG_FUNCTION(this);
    return m_errno;
}
...
SnicSocketImpl::GetSocketType() const
{
    return NS3_SOCK_DGRAM;
}

Ptr<Node>
SnicSocketImpl::GetNode() const
{
    NS_LOG_FUNCTION(this);
    return m_node;
}
...
void
SnicSocketImpl::DeallocateEndPoint()
{
    if (m_endPoint != nullptr)
    {
...
SnicSocketImpl::FinishBind()
{
    ...
    if (m_endPoint != nullptr)
    {
        m_endPoint->SetRxCallback(
            MakeCallback(&SnicSocketImpl::ForwardUp, Ptr<SnicSocketImpl>(this)));
        m_endPoint->SetIcmpCallback(
            MakeCallback(&SnicSocketImpl::ForwardIcmp, Ptr<SnicSocketImpl>(this)));
        m_endPoint->SetDestroyCallback(
            MakeCallback(&SnicSocketImpl::Destroy, Ptr<SnicSocketImpl>(this)));
        done = true;
    }
    if (m_endPoint6 != nullptr)
    {
        m_endPoint6->SetRxCallback(
            MakeCallback(&SnicSocketImpl::ForwardUp6, Ptr<SnicSocketImpl>(this)));
        m_endPoint6->SetIcmpCallback(
            MakeCallback(&SnicSocketImpl::ForwardIcmp6, Ptr<SnicSocketImpl>(this)));
        m_endPoint6->SetDestroyCallback(
            MakeCallback(&SnicSocketImpl::Destroy6, Ptr<SnicSocketImpl>(this)));
        done = true;
    }
    if (done)
    {
...
int
SnicSocketImpl::Bind6()
{
    NS_LOG_FUNCTION(this);
    m_endPoint6 = m_snic->Allocate6();
    if (m_boundnetdevice)
    {
...
int
SnicSocketImpl::Bind(const Address& address)
{
    ...
    if (InetSocketAddress::IsMatchingType(address))
    {
        ...
        {
            m_endPoint = m_snic->Allocate(GetBoundNetDevice(), port);
        }
        else if (ipv4 != Ipv4Address::GetAny() && port == 0)
        {
            m_endPoint = m_snic->Allocate(ipv4);
        }
        else if (ipv4 != Ipv4Address::GetAny() && port != 0)
        {
            m_endPoint = m_snic->Allocate(GetBoundNetDevice(), ipv4, port);
        }
    ...
    else if (Inet6SocketAddress::IsMatchingType(address))
    {
        ...
        {
            m_endPoint6 = m_snic->Allocate6(GetBoundNetDevice(), port);
        }
        else if (ipv6 != Ipv6Address::GetAny() && port == 0)
        {
            m_endPoint6 = m_snic->Allocate6(ipv6);
        }
        else if (ipv6 != Ipv6Address::GetAny() && port != 0)
        {
            m_endPoint6 = m_snic->Allocate6(GetBoundNetDevice(), ipv6, port);
        }
...
int
SnicSocketImpl::Connect(const Address& address)
{
    NS_LOG_FUNCTION(this << address);
...
int
SnicSocketImpl::Send(Ptr<Packet> p, uint32_t flags)
{
...
int
SnicSocketImpl::DoSend(Ptr<Packet> p)
{
...
int
SnicSocketImpl::DoSendTo(Ptr<Packet> p, Ipv4Address dest, uint16_t port, uint8_t tos)
{
    ...
    if (dest.IsBroadcast())
    {
        ...
        for (uint32_t i = 0; i < ipv4->GetNInterfaces(); i++)
        {
            ...
            m_snic->Send(p->Copy(), addri, dest, m_endPoint->GetLocalPort(), port);
    ...
    else if (ipv4->GetRoutingProtocol())
    {
        ...
        header.SetProtocol(SnicL4Protocol::PROT_NUMBER);
...
int
SnicSocketImpl::DoSendTo(Ptr<Packet> p, Ipv6Address dest, uint16_t port)
{
    ...
    else if (ipv6->GetRoutingProtocol())
    {
        Ipv6Header header;
        header.SetDestination(dest);
        header.SetNextHeader(SnicL4Protocol::PROT_NUMBER);
        Socket::SocketErrno errno_;
        Ptr<Ipv6Route> route;
        Ptr<NetDevice> oif = m_boundnetdevice; // specify non-zero if bound to a specific device
        // TBD-- we could cache the route and just check its validity
...
uint32_t
SnicSocketImpl::GetTxAvailable() const
{
    NS_LOG_FUNCTION(this);
    // No finite send buffer is modelled, but we must respect
    // the maximum size of an IP datagram (65535 bytes - headers).
    return MAX_IPV4_SNIC_DATAGRAM_SIZE;
}

int
SnicSocketImpl::SendTo(Ptr<Packet> p, uint32_t flags, const Address& address)
{
...
uint32_t
SnicSocketImpl::GetRxAvailable() const
{
    NS_LOG_FUNCTION(this);
...
Ptr<Packet>
SnicSocketImpl::Recv(uint32_t maxSize, uint32_t flags)
{
...
Ptr<Packet>
SnicSocketImpl::RecvFrom(uint32_t maxSize, uint32_t flags, Address& fromAddress)
{
    NS_LOG_FUNCTION(this << maxSize << flags);
...
int
SnicSocketImpl::GetSockName(Address& address) const
{
    NS_LOG_FUNCTION(this << address);
    if (m_endPoint != nullptr)
    {
...
int
SnicSocketImpl::GetPeerName(Address& address) const
{
    NS_LOG_FUNCTION(this << address);
...
int
SnicSocketImpl::MulticastJoinGroup(uint32_t interface, const Address& groupAddress)
{
...
int
SnicSocketImpl::MulticastLeaveGroup(uint32_t interface, const Address& groupAddress)
{
    NS_LOG_FUNCTION(interface << groupAddress);
    /*
     1) sanity check interface
...
void
SnicSocketImpl::BindToNetDevice(Ptr<NetDevice> netdevice)
{
...
void
SnicSocketImpl::ForwardUp(Ptr<Packet> packet,
                          Ipv4Header header,
                          uint16_t port,
...
void
SnicSocketImpl::ForwardUp6(Ptr<Packet> packet,
                           Ipv6Header header,
                           uint16_t port,
...
void
SnicSocketImpl::ForwardIcmp(Ipv4Address icmpSource,
                            uint8_t icmpTtl,
                            uint8_t icmpType,
                            uint8_t icmpCode,
                            uint32_t icmpInfo)
...
void
SnicSocketImpl::ForwardIcmp6(Ipv6Address icmpSource,
                             uint8_t icmpTtl,
                             uint8_t icmpType,
                             uint8_t icmpCode,
                             uint32_t icmpInfo)
...
void
SnicSocketImpl::SetRcvBufSize(uint32_t size)
{
    m_rcvBufSize = size;
}

uint32_t
SnicSocketImpl::GetRcvBufSize() const
{
    return m_rcvBufSize;
}

void
SnicSocketImpl::SetIpMulticastTtl(uint8_t ipTtl)
{
    m_ipMulticastTtl = ipTtl;
}

uint8_t
SnicSocketImpl::GetIpMulticastTtl() const
{
    return m_ipMulticastTtl;
}

void
SnicSocketImpl::SetIpMulticastIf(int32_t ipIf)
{
    m_ipMulticastIf = ipIf;
}

int32_t
SnicSocketImpl::GetIpMulticastIf() const
{
    return m_ipMulticastIf;
}

void
SnicSocketImpl::SetIpMulticastLoop(bool loop)
{
    m_ipMulticastLoop = loop;
}

bool
SnicSocketImpl::GetIpMulticastLoop() const
{
    return m_ipMulticastLoop;
}

void
SnicSocketImpl::SetMtuDiscover(bool discover)
{
    m_mtuDiscover = discover;
}

bool
SnicSocketImpl::GetMtuDiscover() const
{
    return m_mtuDiscover;
}

bool
SnicSocketImpl::SetAllowBroadcast(bool allowBroadcast)
{
...
bool
SnicSocketImpl::GetAllowBroadcast() const
{
    return m_allowBroadcast;
}

void
SnicSocketImpl::Ipv6JoinGroup(Ipv6Address address,
